...
async def init_db():
    async with aiosqlite.connect(DB_PATH) as db:
        ...
        for migration in [
            ...
            # 기존 중복 제거 (account_mode/symbol/type/allowed_regimes 기준 가장 최근 1건만 유지)
            """DELETE FROM strategies WHERE rowid NOT IN (
                SELECT MAX(rowid) FROM strategies
                GROUP BY account_mode, symbol, type, COALESCE(allowed_regimes, '')
            )""",
