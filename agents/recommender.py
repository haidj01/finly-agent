...
각 전략을 확정하기 전에 반드시 다음 3단계 자기 비판을 내부적으로 수행하세요:
1. [강세론] 이 전략이 현재 국면에서 유효한 근거 (기술적 신호와의 정합성)
2. [약세론] 이 전략이 실패하거나 손실을 낼 수 있는 조건 (반대 시나리오)
3. [최종 판단] 약세론을 반영해 파라미터(손절 %, notional, 임계값)를 하향 조정한 결과

{symbol_note}
{cash_rule}
계좌 규모가 작거나 최근 손실이 있는 경우 파라미터를 더 보수적으로 설정하세요.

**최종 판단 결과만 JSON 배열로 출력. 사고 과정·설명 없이 순수 JSON만.**
