...
자율 매매 에이전트 서비스. Alpaca Trading API와 Claude AI를 연동하여 포트폴리오 분석, 손실 감시, 규칙 기반 자동 매매, AI 기반 전략 추천을 수행합니다. Paper / Live 계좌를 독립적으로 관리합니다.

## 아키텍처

```
finly-agent/
├── main.py                  # FastAPI 앱 + APScheduler 스케줄러 + 내부 토큰 인증 미들웨어
├── db.py                    # SQLite 초기화 (strategies, logs, reports)
├── alpaca_cfg.py            # Alpaca API 설정 (paper/live 모드 전환)
├── agents/
│   ├── portfolio.py         # 포트폴리오 분석 에이전트 (Claude AI)
│   ├── recommender.py       # 전략 추천 에이전트 (Claude AI + 시장 국면)
│   └── watchdog.py          # 손실 임계값 감시 에이전트
├── market/
│   └── regime.py            # 시장 국면 분류 (SPY·QQQ·IWM 앙상블, ADX·MACD 포함)
├── strategies/
...
│   ├── ma.py                # 단순이동평균(SMA) 계산
│   ├── bb.py                # 볼린저밴드 계산
│   ├── adx.py               # ADX / +DI / -DI 계산 (Wilder's smoothing)
│   └── macd.py              # MACD 계산 (EMA 기반)
├── api/
│   ├── agent.py             # /api/agent 라우터 (리포트, 워치독, 추천, 거래 이력)
│   ├── alpaca.py            # /api/alpaca 라우터 (계좌·포지션·주문)
...
[서버 시작]
     ...
     └── APScheduler 구동
           ...
           │                       SPY·QQQ·IWM 250일 바 조회
           │                       → MA/RSI/BB/ADX/MACD 점수 계산 + 앙상블 투표
           ...
           └── 5분 간격 ────────── run_strategy_engine()
                                   ...
                                   현재가 + 포지션 + 인디케이터(RSI/MA/BB/ADX/MACD) 계산
...
- **실행 주기**: 매일 08:00 ET (장 시작 전), 전략 엔진 실행 시마다 호출 (5분 캐시)
- **3개 프록시 앙상블**: SPY(0.5) · QQQ(0.3) · IWM(0.2) 가중 투표로 최종 국면 결정
  - 단일 SPY 의존 대비 섹터 로테이션 포착률 개선
- **5개 지표 점수 기반 분류** (AND 조건 → 앙상블 점수 방식)
  - MA 추세 점수 (25%), RSI 모멘텀 점수 (20%), ADX 추세 강도·방향 점수 (35%), MACD 히스토그램 Z-점수 (20%), BB 변동성 점수
- **신뢰도(confidence)**: 신호 간 합의도 기반 — low/medium/high, 신뢰도 ≤ 0.45이면 ranging 보수 처리
- **서킷 브레이커**: Alpaca API 연속 3회 실패 시 OPEN → 5분 후 HALF_OPEN 자동 복구

| 국면 | 분류 기준 | size_factor |
|------|----------|-------------|
| **하락장** (bearish) | 고변동성 + 강한 하락 추세, 또는 신뢰도 높은 하락 추세 | 0.25 |
| **변동성장** (volatile) | BB폭 점수 > 75 (BB폭 ≈ 8%+) + 하락 추세 아닌 경우 | 0.50 |
| **추세장** (trending) | 신뢰도 높은 상승 추세 (trend_score > 40) | 1.00 |
| **횡보장** (ranging) | 신뢰도 낮거나 위 조건 미해당 (기본값) | 0.75 |

국면 분류 임계값은 환경변수(`REGIME_TREND_STRONG`, `REGIME_VOL_HIGH`, `REGIME_CONF_HIGH_MIN`)로 재정의 가능합니다.

### 3. Portfolio Analysis Agent

- **실행 주기**: 매일 08:30 ET
- Alpaca API로 보유 포지션 및 계좌 현황 병렬 조회
- Claude AI (`CLAUDE_MODEL` 환경변수, 기본값: `claude-sonnet-4-6` + `web_search` 도구)로 한국어 포트폴리오 리포트 생성
  - 포트폴리오 요약 / 종목별 분석(최신 뉴스 반영) / 오늘의 액션 아이템 3개
- 결과를 SQLite `portfolio_reports` 테이블에 영속화

### 4. Strategy Recommender Agent

- **엔드포인트**: `GET /api/agent/regime-recommendations?symbol=AAPL`
- 현재 시장 국면 + 보유 포지션을 Claude AI에 전달하여 맞춤 전략 3가지 추천
- ADX/MACD 신호가 포함된 상세 국면 정보를 프롬프트에 포함
- **재시도 로직**: 최대 3회 시도 (exponential backoff, 총 3회)
- **Fallback**: Claude API 실패 시 국면별 기본 전략 반환 (bearish/volatile/trending/ranging)
- **보안**: symbol sanitization, 응답 JSON 스키마 검증 (type/symbol/action/qty 범위 등)
- `CLAUDE_MODEL` 환경변수로 모델 변경 가능
...
| `CLAUDE_MODEL` | Claude 모델 ID (기본값: `claude-sonnet-4-6`) |
| `FINLY_INTERNAL_TOKEN` | 내부 API 인증 토큰 — 설정 시 모든 API 요청에 `X-Internal-Token` 헤더 필요 |
| `ALLOWED_ORIGIN` | CORS 허용 출처 (기본값: `http://localhost:8000`) |
| `DB_PATH` | SQLite DB 파일 경로 (기본값: `finly_agent.db`) |
| `REGIME_TREND_STRONG` | 추세 강도 임계값 (기본값: `40`, 유효 범위: 1–99) |
| `REGIME_VOL_HIGH` | 고변동성 임계값 (기본값: `75`, 유효 범위: 1–99) |
| `REGIME_CONF_HIGH_MIN` | 신뢰도 강한 신호 임계값 (기본값: `60`, 유효 범위: 1–99) |
...
> `/health`와 `/version`은 `FINLY_INTERNAL_TOKEN` 인증이 면제됩니다.

### Market

```
GET /market/regime                          # 현재 시장 국면 조회 (SPY·QQQ·IWM 앙상블)
...
```
GET /api/agent/regime-recommendations           # 현재 국면 기반 전략 3개 추천
GET /api/agent/regime-recommendations?symbol=AAPL  # 특정 종목 중심으로 전략 추천
```

응답에 `regime`, `regime_label`, `size_factor`, `details`(국면 상세 신호), `recommendations`(추천 전략 배열)이 포함됩니다.
Claude API 실패 시 `fallback_reason` 필드와 함께 국면별 기본 전략이 반환됩니다.
...
GET /api/agent/trade-history
    ...
    &mode=paper      # 필터: paper | live (계좌 모드)
    &source=watchdog # 필터: watchdog | strategy (실행 주체)
```

응답에 `strategy_name`, `strategy_type` 필드가 포함되며 워치독 실행 이력도 함께 조회됩니다.

### Alpaca (Mode-aware Proxy)

현재 거래 모드(paper/live)에 맞는 Alpaca 계좌로 자동 라우팅됩니다.

```
GET  /api/alpaca/account              # 계좌 정보 조회
GET  /api/alpaca/positions            # 보유 포지션 조회
...
- **AI**: Anthropic Claude API (`CLAUDE_MODEL` 환경변수, 기본값: `claude-sonnet-4-6`)
